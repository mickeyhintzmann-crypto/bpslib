...
// Static path (no dynamic [id] segment) to avoid Next.js 16 routing issues.
// Job ID is read from the request body instead.
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Job ID comes from body (not URL) to avoid dynamic-segment routing issues
    const id = asTrimmed(payload.jobId);
    if (!id) {
    ...
    if (id.startsWith("booking:")) {
      ...
      if (existingInvoice?.status === "sent") {
        return NextResponse.json({
          ...
          alreadySent: true,
          invoice: {
            invoiceId: existingInvoice.dinero_invoice_id,
            invoiceNumber: existingInvoice.dinero_invoice_number,
      ...
      try {
        ...
        await supabase.from("employee_dinero_connections")
    ...
    const { data: jobData, error: jobError } = await supabase
      .from("jobs")
      .select("id, title, service, status, lead_id, city, location, address, notes, task_description, assigned_employee_id, lead:lead_id(id,name,email,phone,location,message)")
      .eq("id", id)
