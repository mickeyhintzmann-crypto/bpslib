  ...
  return (
    <section className="space-y-6 rounded-2xl border border-border/70 bg-white/70 p-6 md:p-8">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          <h1 className="font-display text-3xl font-semibold text-foreground">Cases admin</h1>
          <p className="text-sm text-muted-foreground">
            Opret, redigér og slet cases der vises på forsiden og /cases.
          </p>
        </div>
        <div className="flex gap-2">
          ...
          <Button asChild>
            <Link href="/admin/cases/new">Ny case</Link>
          </Button>
        </div>
      </div>
      ...
      <div className="overflow-hidden rounded-2xl border border-border">
        <div className="grid grid-cols-[1.1fr_1.6fr_1fr_1fr_1fr] gap-2 bg-muted/60 px-4 py-3 text-xs font-semibold uppercase text-muted-foreground">
          <span>Oprettet</span>
          <span>Titel</span>
          <span>Kategori</span>
          <span>Lokation</span>
        ...
        <div className="divide-y divide-border/70 bg-white/70">
          {items.length === 0 ? (
            <div className="px-4 py-6 text-sm text-muted-foreground">
              {loading ? "Henter cases..." : "Ingen cases matcher dine filtre."}
            </div>
          ) : (
            items.map((item) => (
              <Link
                ...
                className="grid grid-cols-[1.1fr_1.6fr_1fr_1fr_1fr] gap-2 px-4 py-3 text-sm text-foreground hover:bg-muted/40"
              >
